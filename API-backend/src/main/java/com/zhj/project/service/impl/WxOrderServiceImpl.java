...
public class WxOrderServiceImpl extends ServiceImpl<ProductOrderMapper, ProductOrder> implements ProductOrderService {
    ...
    public ProductOrderVo saveProductOrder(Long productId, UserVO loginUser) {
        ...
        productOrder.setPayType(PayTypeStatusEnum.WX.getValue());
    ...
    public void processingTimedOutOrders(ProductOrder productOrder) {
        ...
        try {
            ...
            // 订单已支付
            if (tradeState.equals(PaymentStatusEnum.SUCCESS.getValue())) {
                this.updateOrderStatusByOrderNo(orderNo, PaymentStatusEnum.SUCCESS.getValue());
                // 用户余额补发
            ...
            if (tradeState.equals(PaymentStatusEnum.NOTPAY.getValue()) || tradeState.equals(PaymentStatusEnum.CLOSED.getValue())) {
            ...
            redisTemplate.delete(PayConstant.QUERY_ORDER_STATUS + orderNo);
        } catch (Exception e) {
    ...
    public String doPaymentNotify(String notifyData, HttpServletRequest request) {
        ...
        try {
            ...
            WxPayOrderNotifyV3Result result = wxPayService.parseOrderNotifyV3Result(notifyData, WxPayUtil.getRequestHeader(request));
            // 解密后的数据
            ...
            if (WxPayConstants.WxpayTradeStatus.SUCCESS.equals(notifyResult.getTradeState())) {
                ...
                if (rLock.tryLock(0, -1, TimeUnit.MILLISECONDS)) {
                    ...
                    if (PaymentStatusEnum.SUCCESS.getValue().equals(productOrder.getStatus())) {
                        redisTemplate.delete(PayConstant.QUERY_ORDER_STATUS + outTradeNo);
